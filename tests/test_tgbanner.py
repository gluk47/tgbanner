...
async def test_delete_casino_messages_banned():
    ...
    for query in BANNED:
        ...
        # Add the query to the assertion failure message
        assert \
...
def test_parse_timedelta():
    ...
    assert parse_timedelta('1d 2h 3m 4s') == datetime.timedelta(days=1, hours=2, minutes=3, seconds=4)
    assert parse_timedelta('2d 3h 4m 5s') == datetime.timedelta(days=2, hours=3, minutes=4, seconds=5)