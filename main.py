...
def parse_timedelta(delta: str):
    ...
    for i, c in enumerate(delta):
        if c.isalpha() and c.isascii():
            ...
            raise ValueError(
                f"Invalid character {c!r} in time delta: {delta!r}")
    if delta[-1].isdigit():
...
def main():
    ...
    parser.add_argument(
        '-t', '--token', default=None, help='Telegram bot token'
    )
    parser.add_argument(
        '-c', '--config', help='Path to the config file', default='config.yaml'
    )
    parser.add_argument(
        '-l', '--loglevel', help='Set log level', default='INFO'
    )
    parser.add_argument(
        '-w', '--window',
    ...
    parser.add_argument(
        '-m', '--limit', help='Spam message limit for banhammer', default=3, type=int
    )
    ...
    app.add_handler(MessageHandler(
        filters.ALL, tg_calls.delete_casino_messages
    ))
    app.run_polling()
